...
image datasets in computer vision contain sub-megapixel images: 0.09
megapixel for ImageNet[@Russakovsky2015] and 0.001 megapixel for
CIFAR-10[@Krizhevsky2009]. In several domains such as remote sensing or
...
task-specific performance increased with larger input image sizes.
...
weighted average from low-dimensional patch
...
et al.[@Tellez2019]. To compress the image to a lower-dimensional space,
they proposed unsupervised learning. The model is trained patch-by-patch
...
lower-dimensional representation per patch. After training, the whole
image is compressed patch-by-patch. A model is subsequently trained on
...
methods are usually insufficient for training with large multi-megapixel
...
Secondly, in tile-based processing, border effects occur at both the
...
::: figure*
![](chpt2_imgs/1dexample.png){width="\\textwidth"}
:::
...
![](chpt2_imgs/constant_input_size.png){width=".8\\columnwidth"}\
...
![](chpt2_imgs/constant_tile_size.png){width=".8\\columnwidth"}\
...
high-resolution images (multiple gigapixels) and only image-level
...
max-pool layer (see Table [1](#tab:imagenetnet){reference-type="ref"
...
$1 \times 10^{-3}$, a mini-batch size of 32 images and weight decay
...
![](chpt2_imgs/streamingvsnormal_rebuttal.png){width="\\columnwidth"}\
![](chpt2_imgs/streamingvsnormal_accuracy_rebuttal.png){width="\\columnwidth"}
...
consists of 500 hematoxylin and eosin (H&E) stained whole-slide images
...
physical receptive field size before the last max-pool layer is kept
...
resulting image was either randomly cropped or zero-padded to the
...
smooth L1 loss[@Girshick:2015:FR:2919332.2920125]. The mini-batch size
...
image-level regression with a score of 0.570. Note that the first entry
...
macro-metastases ($>$ 2 mm), micro-metastases ($\leq$ 2.0 mm or $>$ 200
cells in a single cross-section), and isolated tumor cells ($\leq$ 0.2
mm or $<$ 200 cells in a single cross-section). We evaluate the slide
...
activation were calculated over ten mini-batches by keeping track of the
...
cell-dense and cancerous regions. There is a trend in which the higher
...
tissue.](chpt2_imgs/comparison_tupac.jpg){#figure:saliency width="\\columnwidth"}
...
recognize.](chpt2_imgs/comparisoncam_rebuttal.jpg){#figure:saliency
width="\\columnwidth"}
...
::: figure*
![](chpt2_imgs/tupac_rebuttal.png){width="\\textwidth"}
:::
...
[2](#figure:saliency){reference-type="ref"
reference="figure:saliency"}). The image-level trained CAMELYON17
network shows highlights corresponding to the ground truth pixel-level
annotation of a breast cancer metastasis. The TUPAC16 network shows
highlights in cell-dense regions.
...
pre-trained network (e.g., on ImageNet) and fine-tune for a shorter
period.
...
[^1]: By convention we used the term *convolution* although the
    ...
    frameworks (e.g., TensorFlow, PyTorch) is a cross-correlation.

[^2]: Zero-padding the tiles before the convolution does not help
